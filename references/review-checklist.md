...
- Internal implementation, formulas, acceptance logic, and technical detail are removed.
- Structure matches selected template `A/B/C`.
- Stage 2 includes `【宣发建议】`.
- `【宣发建议】` is one of:
  - `官网产品动态`
  - `官网产品动态 + 月报`
  - `官网产品动态 + 培训`
...
- Summary can answer: what was not possible before, and what is possible now.
...
- Bullets with same capability are merged.
...
- For model/algorithm/engine/protocol updates, summary value is customer-actionable outcome, not technical expansion wording.
...
- Promotion recommendation follows the rules:
  - customer-visible change -> `官网产品动态`
  - clear value / clear scenario / capability signal -> add `月报`
  - support or ops explanation needed -> add `培训`
